...
\row{openable suit\footnote{Opening a suit requires 5 of a major or 4 of a minor}}
  ...
  \bid(23,lpurple){2\C\footnote{Strong, artificial}}\\%\hline
\row{6 card suit}
...
\row{fit (3 trump)}
  ...
  \bid(11-12,lyellow){limit raise (3M)}
  \bid(13,lpurple){Jacoby 2NT\savenote{\reqfour}{Jacoby 2NT and splinter bids require
4-card trump support}}\\
\row{\ldots\ + 1$^{\underline{\mathrm{tn}}}$/void}
  \bid(6-12,white){\ditto{2in}}
  \bid(13,lred){Splinter bid\footnote{Splinter bid is a double-jump (non-game)
bid in new suit, showing a singleton.}}\\
%\row{5 trump fit}
%  \bid(7-10){preempt to game (4M)}
\row{four-card suit\footnote{Bidding 2\H\ as 2/1 requires a five-card suit}}
  \bid(6-10,lblue){1NT\savenote{\ntresp}{1NT response/rebid denies a biddable 1-level major}}
  \bid(11-13,lgreen){2/1}\\
\row{\phantom{m}(denies fit)}
  \bid(6-13,laqua){1/1}\\
\row{six-card suit}
...
\section{Responses to 1 of a minor}
Includes \textbf{Inverted Minor Raises}.\\

\noindent\begin{ptable}\headings{2-6|6-10|11+}
\row{fit (4 trump)}
  \bid(6-10,lyellow){3m\footnote{Prefer 1NT with a bare 10-point raise}}
  \bid(11,lred){2m}\\
\row{four-card suit\footnote{After a minor suit opening, bid any 4-card
majors at the one level up the line, rather than showing fit}}
  \bid(6-10,lblue){1NT\footnotemark[\ntresp]}
  \bid(11,lgreen){2/1}\\
\row{}
  \bid(6-11,laqua){1/1}\\
\row{six-card suit}
  \bid(2-6,lorange){jump shift\footnotemark[\weakshift]}
  \bid(6-11,white){\ditto{1in}}\\
\end{ptable}

\section{Responses to 1NT}
\noindent\begin{ptable}\headings{0-7|8|9-10|11-15|16+}
\row{(semi)balanced}
  \bid(9-10,lblue){2NT}
  \bid(11-15,lred){3NT}
  \bid(16,lpurple){4NT}\\
